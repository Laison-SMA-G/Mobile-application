...
const ProductDetails = ({ route, navigation }) => {
  ...
  const handleAddToCart = () => {
    ...
    if (currentQuantityInCart < actualStock) {
      ...
      setTimeout(() => setSuccessToastVisible(false), 1500);
    } else {
  ...
  const handleProceedToCheckout = () => {
    ...
    navigation.navigate("Checkout", { items: [{ ...product, quantity }], total: parseFloat(product.price) * quantity });
    setBuyNowModalVisible(false);
    setQuantity(1);
  };
          ...
          <View style={styles.infoRowContainer}>
            ...
            <View style={styles.infoBox}>
              ...
              <Text style={[styles.infoText, isOutOfStock && styles.outOfStockText]}>
                {isOutOfStock ? "Out of Stock" : `${actualStock} in stock`}
              </Text>
            </View>
          </View>
